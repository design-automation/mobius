        ...
        if (_child_len > 1) {
          ...
          var oppositeFlowProperty = isUsingColumnFlow ? 'right' : 'bottom';
            ...
            for (_i = 0; _i < _child_len; ++_i) {
                ...
                if(_i == 1){
                    var area = angular.element(_childens[_i]).css(flowProperty, step * _i + '%').css(oppositeFlowProperty, 100 - step * (_i + 2) + '%');
                    if (_i < _child_len - 1) {
                        var bar = angular.element(splitBarElem_htmlTemplate).css(flowProperty, step * (_i + 2) + '%');
                        area.after(bar);
                    }
                }

                if(_i == 2){
                    var area = angular.element(_childens[_i]).css(flowProperty, step * (_i+1) + '%').css(oppositeFlowProperty, 100 - step * (_i + 2) + '%');
                    if (_i < _child_len - 1) {
                        var bar = angular.element(splitBarElem_htmlTemplate).css(flowProperty, step * (_i + 1) + '%');
                        area.after(bar);
                    }
